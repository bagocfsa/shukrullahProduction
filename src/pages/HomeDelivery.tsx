...
import { PlusIcon, MinusIcon, TrashIcon, MapPinIcon, PhoneIcon, UserIcon } from '@heroicons/react/24/outline';
...
const HomeDelivery: React.FC = () => {
  ...
  const saveOrderToStorage = (order: DeliveryOrder) => {
    const existingOrders = JSON.parse(localStorage.getItem('shukrullah-delivery-orders') || '[]');
    existingOrders.push(order);
    localStorage.setItem('shukrullah-delivery-orders', JSON.stringify(existingOrders));
  };

  const generateWhatsAppMessage = (order: DeliveryOrder) => {
    ...
    message += `🏪 *Ordered via:* Shukrullah Nigeria Ltd\n`;
    message += `📞 *Shop Contact:* +234-901-928-6029`;
  ...
  const submitOrder = () => {
    ...
    alert('Order submitted! WhatsApp message has been prepared for sending.');
  };
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Complete Your Delivery Order</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Home Delivery Service</h1>
        <p className="text-gray-600 mt-2">Order products for delivery anywhere in Minna and surrounding areas</p>
      </div>
      ...
      {deliveryItems.length > 0 && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">Your Order ({deliveryItems.length} items)</h3>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {deliveryItems.map((item) => (
              <div key={item.productId} className="flex items-center p-3 border border-gray-200 rounded-lg">
                <img src={item.image} alt={item.productName} className="w-12 h-12 object-cover rounded mr-3" />
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product) => {
            const price = getProductPrice(product.price, product.physicalShopPrice);
